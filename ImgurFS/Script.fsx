...
let SaveStreamFromHttpResponse (targetPath:string) (fileName:string) (index:int) (response:HttpResponseWithStream) =
...
let DownloadLinks targetPath (images : seq<string>) =
    ...
    |>Seq.iter(fun (image, stream) -> printf "[%d / %d] " counter imageLength
                                      SaveStreamFromHttpResponse targetPath image counter stream
                                      counter <- counter + 1)
...
let GetAlbumNameOrDefault (album:JsonValue) (defaultName:string) =
...
let downloadAlbum albumHash targetFolder=
    let album = sprintf "https://api.imgur.com/3/album/%s" albumHash |> DownloadAlbumData    
    GetImageLinksOfAlbum album |> DownloadLinks (Path.Combine(targetFolder, GetAlbumNameOrDefault album albumHash))
